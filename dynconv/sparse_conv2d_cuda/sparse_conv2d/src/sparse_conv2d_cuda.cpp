...
int MaskedCol2imForwardLaucher1(const at::Tensor col, const int height,
                               const int width, const int channels,
                               const at::Tensor mask_h_idx,
                               const at::Tensor mask_w_idx,
...
int masked_im2col_forward_cuda(const at::Tensor im, const at::Tensor mask_h_idx,
  ...
  MaskedIm2colForwardLaucher(im, height, width, channels, kernel_h, kernel_w,stride_h,stride_w,
                             pad_h, pad_w, mask_h_idx, mask_w_idx, mask_cnt, col);
...
int masked_col2im_forward1_cuda(const at::Tensor col,
                               const at::Tensor mask_h_idx,
                               const at::Tensor mask_w_idx,
                               const at::Tensor c_mask_idx,
  ...
  MaskedCol2imForwardLaucher1(col, height, width, channels, mask_h_idx,
                              mask_w_idx,c_mask_idx, mask_cnt, im);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("masked_col2im_forward1", &masked_col2im_forward1_cuda,
        "masked_col2im forward1 (CUDA)");
}
